...
int main( int argc, char *argv[] )
{
    ...
    if(commandSet->flag) {
        ...
        if(process_id > 0) {
            ...
            for(int c = 0; commandSet->commands[c]; c++) {
                cmd command = commandSet->commands[c];
                pid_t pid = fork(); // make grandchildren
                ...
                pid_t id;
                for(int c=0; commandSet->commands[c]; c++){
                    id = wait3(&status, WNOHANG, NULL); //issue wait call, but non-blocking, so that the system knows
                }
                exit(0); //kill the child and make zombie grandchildren
            }
        }
    } else {
        for(int c = 0; commandSet->commands[c]; c++) {
